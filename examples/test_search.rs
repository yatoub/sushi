        ...
        Ok(config) => {
            ...
            match config.resolve() {
                Ok(servers) => {
                    ...
                    let name_matches: Vec<_> = servers.iter()
                        .filter(|s| s.name.to_lowercase().contains(&name_search.to_lowercase()))
                        .collect();
                    println!("Search by name '{}': {} results", name_search, name_matches.len());
                    ...
                    let host_matches: Vec<_> = servers.iter()
                        .filter(|s| s.host.to_lowercase().contains(&host_search.to_lowercase()))
                        .collect();
                    println!("\nSearch by host '{}': {} results", host_search, host_matches.len());
                    ...
                    let combined_matches: Vec<_> = servers.iter()
                        .filter(|s| {
                            s.name.to_lowercase().contains(&combined_search.to_lowercase()) ||
                            s.host.to_lowercase().contains(&combined_search.to_lowercase())
                        })
                        .collect();
                    println!("\nSearch by name OR host '{}': {} results", combined_search, combined_matches.len());
                    for s in combined_matches.iter().take(5) {
                        println!("  - {}/{}/{} -> {}", s.group_name, s.env_name, s.name, s.host);
                    }
                },
                Err(e) => {
